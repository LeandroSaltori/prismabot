...
Em ambos os planos a plataforma é **plataforma de atendimento**: você usa sua própria marca, seu domínio e suas cores. A diferença está na escala — o plano **Uso Próprio** opera com uma única conta, enquanto o plano **cliente** permite criar contas isoladas para múltiplos clientes (multi-tenant) e comercializá-las como SaaS.
...
**Prazo de instalação pela equipe:** 2 a 5 dias úteis após agendamento e envio das credenciais da servidor e do domínio. Se preferir mais agilidade, o auto-instalador está disponível com download liberado imediatamente após confirmação do pagamento.
...
**cliente (SaaS / plataforma de atendimento)**
...
**A principal diferença:** o Uso Próprio centraliza o atendimento em uma única conta. O plano de cliente (Multi-Tenant) permite criar contas isoladas para múltiplos clientes, separando a gestão e os números conectados de cada um. Ambos os planos não possuem limite de números e usuários.

[→ Ver valores e adquirir licença](https://prismabot.zdg.com.br/#oferta)

#### Como funciona a cliente (plataforma de atendimento)

Com o Plano cliente, você:
...
### Servidor (servidor) e responsabilidades

**O Prismabot é em nuvem.** Você contrata e mantém o servidor (servidor) onde o sistema roda. A servidor não está inclusa na licença — é um custo de infraestrutura seu, pago diretamente ao provedor de hospedagem.
...
[→ Pré-requisitos de instalação (especificações mínimas da servidor)](/diretrizes-e-politicas/pre-requisitos-de-instalacao-e-utilizacao)
...
**IAs suportadas:** ChatGPT (OpenAI), Claude (Anthropic), Gemini (Google), Grok (xAI), Deepseek, Qwen — e modelos em nuvem via Ollama e LM Studio. Basta inserir sua chave de API.
...
Não. O Prismabot não impõe limites de software. A capacidade depende dos recursos do seu servidor (servidor).
