...
A seção de **Automação** reúne as ferramentas que permitem ao administrador configurar fluxos automáticos de atendimento, disparos programados e respostas a eventos externos — reduzindo intervenção manual e garantindo consistência no relacionamento com o cliente.
...
* **Chat Flow** — visão geral, gerenciamento de fluxos e como criar o primeiro bot
* **Configurações do Fluxo** — saudação, palavra-chave de ativação, timeout, máximo de tentativas, horário de atendimento e distribuição automática de tickets
* **Interações Disponíveis** — todos os blocos que podem ser usados no fluxo: mensagens, mídia, roteamento, CRM, integrações externas e utilitários
* **Distribuição Sequencial** — como configurar a distribuição em fila sequencial para tickets gerados pelo bot
* **Exemplo: Menu de Departamentos** — fluxo de menu com opções numéricas roteando para filas diferentes
* **Exemplo: Fluxo com Horário de Atendimento** — como combinar verificação de horário com mensagens distintas para dentro e fora do expediente
* **Exemplo: Qualificação de Lead com Captura de Dados** — coleta de nome, e-mail e outros campos antes de direcionar ao atendente
* **Boas Práticas e Solução de Problemas** — erros comuns, checklist de revisão e orientações para fluxos mais robustos
