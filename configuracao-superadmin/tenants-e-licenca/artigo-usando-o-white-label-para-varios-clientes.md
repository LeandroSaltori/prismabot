...
# Artigo: Usando o White-Label para Vários Clientes

Esta página é um guia completo para entender como o modelo White-Label do Prismabot funciona em cenários com múltiplos clientes. O objetivo é esclarecer a relação entre sua licença, as instalações, a personalização de marca e as regras de uso da plataforma.
...
* **Múltiplas Instalações (VPS):** Sua única licença permite que você instale o Prismabot em múltiplos servidores (VPS), desde que as regras de propriedade da infraestrutura sejam seguidas.
...
* **Uma Marca (White-Label) por Instalação:** A personalização de marca (logo, cores, nome) é definida no nível do Super Admin e se aplica a **toda aquela instalação** e a todos os tenants dentro dela.
...
* **Como Funciona:** Você tem **uma única instalação** do Prismabot em um único servidor (VPS). Dentro desta instalação, você cria múltiplos tenants, um para cada cliente seu.
* **Personalização:** Você configura o White-Label com a **sua própria marca** no painel Super Admin. Todos os seus clientes acessarão a plataforma através de um subdomínio seu (ex: app.suaagencia.com) e verão a sua marca.

**Vantagens:**

* **Gestão Simplificada:** Você gerencia, atualiza e mantém um único servidor.
* **Custo de Infraestrutura Menor:** O custo se limita a um único VPS (que pode precisar de upgrade conforme o número de clientes cresce).
...
* **Como Funciona:** Você realiza **múltiplas instalações** do Prismabot, cada uma em um **servidor (VPS) separado**. Cada instalação será dedicada a um único cliente final.

Você (o assinante) deve ser o proprietário ou o responsável direto pela infraestrutura (VPS) onde cada instalação está rodando

* **Personalização:** Como cada instalação tem seu próprio painel Super Admin, você pode configurar o White-Label de **cada instalação com a marca específica daquele cliente**.

**Exemplo:**

* **Instalação 1 (VPS 1):** Para o Cliente A. Acessível em clientea.suaagencia.com. Marca, logo e cores do Cliente A.
* **Instalação 2 (VPS 2):** Para o Cliente B. Acessível em clienteb.suaagencia.com. Marca, logo e cores do Cliente B.

**Vantagens:**

* **Personalização Total:** Cada cliente tem uma experiência 100% White-Label com sua própria marca.
...
* **Custo de Infraestrutura Maior:** Cada instalação requer seu próprio VPS, multiplicando os custos.
...
2. **Você (o assinante) deve ser o proprietário ou o responsável direto pela infraestrutura (VPS)** onde cada instalação está rodando.
